<template>
  <main class="mx-auto grid w-full max-w-[430px] gap-2.5 px-2.5 pb-24 pt-2.5">
    ...
    <div v-if="showDeleteConfirmation" class="fixed inset-0 z-20 flex items-end justify-center bg-[rgba(27,31,44,0.45)]" role="alertdialog" aria-modal="true" aria-labelledby="delete-confirm-title" @click="showDeleteConfirmation = false">
      <section class="w-full max-w-[430px] rounded-t-2xl bg-[rgba(255,252,244,0.98)] p-3 shadow-[0_-10px_40px_rgba(82,61,20,0.18)]" @click.stop>
        <div class="mb-3.5 flex items-center gap-3">
          <button aria-label="Close" class="border-0 bg-transparent text-2xl text-[#5d4930]" type="button" :disabled="booksStore.serverWriteInFlight" @click="showDeleteConfirmation = false">x</button>
        ...
        <div class="flex flex-col gap-2">
          <button
            class="w-full rounded-full border-0 bg-[#c23c37] px-3.5 py-2.5 text-center font-bold text-[#fff8ea] shadow-[0_12px_24px_rgba(194,60,55,0.18)] disabled:opacity-50"
            type="button"
            :disabled="booksStore.serverWriteInFlight"
            @click="confirmDeleteBook"
          >
            {{ booksStore.serverWriteInFlight ? 'Deleting...' : 'Confirm Delete' }}
          </button>
          <button
            class="w-full rounded-full border border-[#6553281f] bg-transparent px-3.5 py-2.5 text-center font-bold text-[#342716] disabled:opacity-50"
            type="button"
            :disabled="booksStore.serverWriteInFlight"
...
function deleteSelectedBook() {
  const targetBook = booksStore.getBookById(activeMenuBook.value)
  if (!targetBook) {
    return
  }

  bookToDelete.value = targetBook

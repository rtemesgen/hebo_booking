  ...
  <main class="mx-auto grid w-full max-w-[430px] gap-2.5 px-2.5 pb-24 pt-2.5">
    ...
    <div v-if="showInviteSheet" class="fixed inset-0 z-20 flex items-end justify-center bg-[rgba(27,31,44,0.45)]" @click="closeInviteSheet">
      <section class="grid w-full max-w-[430px] gap-4 rounded-t-2xl bg-[rgba(255,252,244,0.98)] p-3 shadow-[0_-10px_40px_rgba(82,61,20,0.18)]" @click.stop>
        <div class="mb-1 flex items-center gap-3">
          <button aria-label="Close add member" class="border-0 bg-transparent text-2xl text-[#5d4930]" type="button" @click="closeInviteSheet">x</button>
    ...
    <div v-if="showDeleteConfirmSheet" class="fixed inset-0 z-20 flex items-end justify-center bg-[rgba(27,31,44,0.45)]" @click="closeDeleteConfirmSheet">
      ...
      >
        ...
        <p class="mb-5 text-[0.88rem] text-[#5d4930]">
          Are you sure you want to delete <strong>{{ bookToDelete?.name }}</strong>? This action will permanently remove all records within this book and cannot be undone.
        </p>

        <div class="flex flex-col gap-2">
          <button
            class="w-full rounded-full border-0 bg-[#c23c37] px-3.5 py-2.5 text-center font-bold text-[#fff8ea] shadow-[0_12px_24px_rgba(194,60,55,0.18)] disabled:opacity-70"
            type="button"
            :disabled="serverWriteInFlight"
            @click="confirmDeleteBook"
          >
            {{ serverWriteInFlight ? 'Deleting...' : 'Yes, Delete Book' }}
          </button>
          <button
            class="w-full rounded-full border border-[#6553281f] bg-[#fffdfa] px-3.5 py-2.5 text-center font-bold text-[#5d4930]"
            type="button"

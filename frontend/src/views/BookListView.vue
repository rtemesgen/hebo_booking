  ...
  <main class="mx-auto grid w-full max-w-[430px] gap-2.5 px-2.5 pb-24 pt-2.5">
    ...
    <div v-if="activeMenuBook" class="fixed inset-0 z-20 flex items-start justify-end bg-[rgba(27,31,44,0.45)] px-4 pt-40" @click="activeMenuBook = null">
      <section class="w-[min(220px,calc(100vw-1.4rem))] rounded-[14px] bg-[rgba(255,252,244,0.98)] p-2 shadow-[0_-10px_40px_rgba(82,61,20,0.18)]" @click.stop>
        ...
        <button class="w-full rounded-[10px] bg-transparent px-2.5 py-2 text-left text-[0.85rem] text-[#c23c37]" type="button" @click="openDeleteConfirm">Delete Book</button>
      </section>
    </div>

    <div v-if="showDeleteConfirmSheet" class="fixed inset-0 z-20 flex items-end justify-center bg-[rgba(27,31,44,0.45)]" @click="closeDeleteConfirm">
      <section class="w-full max-w-[430px] rounded-t-2xl bg-[rgba(255,252,244,0.98)] p-3 shadow-[0_-10px_40px_rgba(82,61,20,0.18)]" @click.stop>
        <div class="mb-3.5 flex items-center gap-3">
          <button class="border-0 bg-transparent text-2xl text-[#5d4930]" type="button" aria-label="Close delete confirmation" @click="closeDeleteConfirm">x</button>
          <h2 class="m-0 text-[0.95rem]">Delete Book</h2>
        </div>
        <p class="mb-5 mt-0 text-[0.85rem] text-[#5d4930]">
          Are you sure you want to delete <strong>{{ activeMenuBookName }}</strong>? This action cannot be undone and all records in this book will be lost.
        </p>
...
const activeMenuBookName = computed(() => {
  const id = bookToDelete.value || activeMenuBook.value
  if (!id) return ''
  return booksStore.getBookById(id)?.name || ''
})
...
function openDeleteConfirm() {
  bookToDelete.value = activeMenuBook.value
...
async function deleteSelectedBook() {
  ...
  const removed = await booksStore.removeBook(id)
  showDeleteConfirmSheet.value = false
  bookToDelete.value = null

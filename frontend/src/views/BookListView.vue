  ...
  <main class="mx-auto grid w-full max-w-[430px] gap-2.5 px-2.5 pb-24 pt-2.5">
    ...
    <div v-if="showInviteSheet" class="fixed inset-0 z-20 flex items-end justify-center bg-[rgba(27,31,44,0.45)]" @click="closeInviteSheet">
      <section class="grid w-full max-w-[430px] gap-4 rounded-t-2xl bg-[rgba(255,252,244,0.98)] p-3 shadow-[0_-10px_40px_rgba(82,61,20,0.18)]" @click.stop>
        <div class="mb-1 flex items-center gap-3">
          <button aria-label="Close invite member" class="border-0 bg-transparent text-2xl text-[#5d4930]" type="button" @click="closeInviteSheet">x</button>
    ...
    <div v-if="showDeleteConfirmSheet" class="fixed inset-0 z-20 flex items-end justify-center bg-[rgba(27,31,44,0.45)]" @click="closeDeleteConfirmSheet">
      <section
        ...
        @click.stop
      >
        <h2 id="delete-confirm-title" class="mb-2 mt-0 text-lg font-bold text-[#342716]">Delete Book?</h2>
        <p class="mb-6 text-[0.9rem] text-[#7a715f]">
          Are you sure you want to delete <strong>{{ bookToDelete?.name }}</strong>? This will also remove all its records. This action cannot be undone.
        </p>
        <div class="grid gap-2">
          <button
            class="w-full rounded-full bg-[#c23c37] py-3 font-bold text-white shadow-lg"
            type="button"
          ...
          <button
            class="w-full rounded-full border border-[#6553281f] bg-[#fffdfa] py-3 font-bold text-[#342716]"
            type="button"

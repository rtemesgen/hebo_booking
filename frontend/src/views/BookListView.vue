<template>
  <main class="mx-auto grid w-full max-w-[430px] gap-2.5 px-2.5 pb-24 pt-2.5">
    ...
    <div v-if="showDeleteBookSheet" class="fixed inset-0 z-20 flex items-end justify-center bg-[rgba(27,31,44,0.45)]" @click="showDeleteBookSheet = false">
      <section class="w-full max-w-[430px] rounded-t-2xl bg-[rgba(255,252,244,0.98)] p-3 shadow-[0_-10px_40px_rgba(82,61,20,0.18)]" @click.stop>
        <div class="mb-3.5 flex items-center gap-3">
          <button aria-label="Close" class="border-0 bg-transparent text-2xl text-[#5d4930]" type="button" @click="showDeleteBookSheet = false">x</button>
          <h2 class="m-0 text-[0.95rem]">Delete book</h2>
        </div>
        <p class="mb-5 mt-0 text-[0.85rem] text-[#5f5b48]">Delete this book and all its records? This cannot be undone.</p>
        <button
          class="w-full rounded-[12px] border-0 bg-[#d34238] py-3 text-[0.85rem] font-bold text-[#fff8ea] disabled:opacity-50"
          type="button"
...
const showDeleteBookSheet = ref(false)
...
function deleteSelectedBook() {
  ...
  showDeleteBookSheet.value = true
}
...
async function confirmDeleteBook() {
  ...
  showDeleteBookSheet.value = false
  activeMenuBook.value = null
}

<template>
  <main class="mx-auto grid w-full max-w-[430px] gap-2.5 px-2.5 pb-24 pt-2.5">
    ...
    <div v-if="showDeleteConfirmSheet" class="fixed inset-0 z-30 flex items-end justify-center bg-[rgba(27,31,44,0.45)]" role="alertdialog" aria-modal="true" aria-labelledby="delete-confirm-title" @click="!serverWriteInFlight && closeDeleteConfirmSheet()">
      <section class="grid w-full max-w-[430px] gap-4 rounded-t-2xl bg-[rgba(255,252,244,0.98)] p-4 shadow-[0_-10px_40px_rgba(82,61,20,0.18)]" @click.stop>
        <div class="text-center">
          <h2 id="delete-confirm-title" class="mb-2 mt-0 text-lg font-bold text-[#342716]">Delete '{{ bookToDelete?.name }}'?</h2>
          <p class="m-0 text-[0.85rem] text-[#7a715f]">This action cannot be undone. All records in this book will be permanently removed.</p>
        </div>

        <div class="grid gap-2">
          <button
            class="w-full rounded-full border-0 bg-[#c23c37] px-4 py-3 text-[0.85rem] font-bold text-white shadow-lg disabled:opacity-50"
            type="button"
            :disabled="serverWriteInFlight"
            @click="confirmDeleteBook"
          >
            {{ serverWriteInFlight ? 'Deleting...' : 'Delete Book' }}
          </button>
          <button
            class="w-full rounded-full border border-[#6553281f] bg-transparent px-4 py-3 text-[0.85rem] font-bold text-[#5d4930] disabled:opacity-50"
            type="button"
            :disabled="serverWriteInFlight"

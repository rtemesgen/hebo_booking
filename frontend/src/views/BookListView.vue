  ...
  <main class="mx-auto grid w-full max-w-[430px] gap-2.5 px-2.5 pb-24 pt-2.5">
    ...
    <div v-if="showDeleteConfirmSheet" class="fixed inset-0 z-20 flex items-end justify-center bg-[rgba(27,31,44,0.45)]" @click="closeDeleteConfirmSheet">
      <section class="w-full max-w-[430px] rounded-t-2xl bg-[rgba(255,252,244,0.98)] p-3 shadow-[0_-10px_40px_rgba(82,61,20,0.18)]" role="alertdialog" aria-modal="true" aria-labelledby="delete-confirm-title" @click.stop>
        ...
        <p class="mb-4 mt-0 text-[0.85rem] text-[#7a715f]">Are you sure you want to delete this book? This action cannot be undone and all records within this book will be lost.</p>
        <button class="w-full rounded-full border-0 bg-[#c23c37] px-3.5 py-2.5 text-center text-[#fff8ea] shadow-[0_12px_24px_rgba(194,60,55,0.18)]" type="button" @click="deleteSelectedBook">
          Delete Book
        </button>
      </section>
    </div>
...
function confirmDeleteBook() {
  const target = booksStore.getBookById(activeMenuBook.value)
  if (!target) {
    return
  }
  bookToDelete.value = target

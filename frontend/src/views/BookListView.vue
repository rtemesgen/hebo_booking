    ...
    <div v-if="showDeleteConfirmSheet" class="fixed inset-0 z-20 flex items-end justify-center bg-[rgba(27,31,44,0.45)]" role="alertdialog" aria-modal="true" aria-labelledby="delete-confirm-title" @click="closeDeleteConfirmSheet">
      <section class="w-full max-w-[430px] rounded-t-2xl bg-[rgba(255,252,244,0.98)] p-3 shadow-[0_-10px_40px_rgba(82,61,20,0.18)]" @click.stop>
        <div class="mb-3.5 flex items-center gap-3">
          <button aria-label="Close confirmation" class="border-0 bg-transparent text-2xl text-[#5d4930]" type="button" @click="closeDeleteConfirmSheet">x</button>
          <h2 id="delete-confirm-title" class="m-0 text-[0.95rem]">Confirm Deletion</h2>
        </div>
        <p class="mb-4 text-[0.85rem] text-[#342716]">Are you sure you want to delete <strong>{{ bookToDelete?.name }}</strong>? This action will remove all records in this book and cannot be undone.</p>
        <div class="grid gap-2">
          <button class="w-full rounded-full border-0 bg-[#c23c37] px-3.5 py-2.5 text-center text-white shadow-[0_12px_24px_rgba(194,60,55,0.18)]" type="button" @click="performDeleteBook">
            Delete Permanently
          </button>
          <button class="w-full rounded-full border border-[#6553281f] bg-white px-3.5 py-2.5 text-center text-[#342716]" type="button" @click="closeDeleteConfirmSheet">
            Keep Book
          </button>
        </div>
      </section>
